...
void time_crack(uint8_t passwd_hash[16], FILE *pFile) {
	double start, end;
	double diff;
  	start = MPI_Wtime();
    MPI_Init(NULL, NULL);
    ...
    int quot = L / world_size;
    int rem = L % world_size;
    const int s = quot*world_rank + MIN(world_rank, rem);
    const int SUB_L = quot+(world_rank < rem ? 1 : 0);
	
	int pwd=-1;
    int rec=0;
	uint8_t pattern[9], hash[16];
    // printf("rand: %d, %d~%d\n", world_rank, s, s+SUB_L);
    if (world_rank == ROOT)
        MPI_Irecv(&rec, 1, MPI_INT, MPI_ANY_SOURCE, 5/*tag*/, MPI_COMM_WORLD, &req);
    else
        MPI_Ibcast(&rec, 1, MPI_INT, ROOT, MPI_COMM_WORLD, &req);
    int idx;
	for (idx = s; idx < s+SUB_L; idx++) {
        int result;
        MPI_Test(&req, &result, &stat);
        if (result) {
            if (world_rank == ROOT) {
                MPI_Ibcast( &result, 1, MPI_INT, ROOT, MPI_COMM_WORLD, &req2);
                MPI_Wait(&req2, &stat);
            }
            break;
        }
        snprintf(pattern, 9, "%08d", idx);
        md5(pattern, 8, hash);
        if (!strncmp(hash, passwd_hash, 16)) {
            pwd = idx;
            if (world_rank == ROOT) {
                MPI_Ibcast( &result, 1, MPI_INT, ROOT, MPI_COMM_WORLD, &req2);
                MPI_Wait(&req2, &stat);
            } else {
                MPI_Send(&result, 1, MPI_INT, ROOT, 5, MPI_COMM_WORLD);
            }
            break;
        } 
	}
    // printf("%d end at %d\n", world_rank, idx);
    int sum;
    MPI_Reduce(&pwd, &sum, 1, MPI_INT, MPI_SUM, ROOT, MPI_COMM_WORLD);
    if (world_rank == ROOT) {
        if (sum > -world_size) {
            // printf("%d\n", sum+world_size-1);
        }
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1) {
        if(argv[1][0] == '0') {
            time_crack(passwd_hash_0, NULL);
        }
        if(argv[1][0] == '1') {
            time_crack(passwd_hash_1, NULL);
        }
        if(argv[1][0] == '2') {
            time_crack(passwd_hash_2, NULL);
        }
        if(argv[1][0] == '3') {
            time_crack(passwd_hash_3, NULL);
        }
        if(argv[1][0] == '4') {
            time_crack(passwd_hash_4, NULL);
        }
        if(argv[1][0] == '5') {
            time_crack(passwd_hash_5, NULL);
        }
        if(argv[1][0] == '6') {
            time_crack(passwd_hash_6, NULL);
        }
        if(argv[1][0] == '7') {
            time_crack(passwd_hash_7, NULL);
        }
        if(argv[1][0] == '8') {
            time_crack(passwd_hash_8, NULL);
        }
        if(argv[1][0] == '9') {
            time_crack(passwd_hash_9, NULL);
        }
    }
	...
	for( int thread_num = 1 ; thread_num <= 4 ; ++ thread_num) {
